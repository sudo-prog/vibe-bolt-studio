...
export const DEFAULT_MODEL = 'zen-coder-free';
...
const PROVIDER_LIST: ProviderInfo[] = [
  {
    name: 'OpenCode',
    staticModels: [
      { name: 'zen-coder-free', label: 'Zen Coder (Free)', provider: 'OpenCode', maxTokenAllowed: 8000 },
      { name: 'zen-coder', label: 'Zen Coder', provider: 'OpenCode', maxTokenAllowed: 8000 },
    ],
    getDynamicModels: getOpenCodeModels,
    getApiKeyLink: 'https://opencode.ai/settings',
    labelForGetApiKey: 'Get OpenCode API Key',
    icon: 'i-ph:code',
  },
  {
    name: 'Anthropic',
...
async function getOpenCodeModels(apiKeys?: Record<string, string>, settings?: IProviderSetting): Promise<ModelInfo[]> {
  try {
    const baseUrl = settings?.baseUrl || import.meta.env.OPENCODE_API_BASE_URL || 'https://api.opencode.ai/v1';
    let apiKey = '';

    if (apiKeys && apiKeys.OpenCode) {
    ...
    console.error('Error getting OpenCode models:', e);
    return [];
  }
}
