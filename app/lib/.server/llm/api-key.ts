...
export function getAPIKey(cloudflareEnv: Env, provider: string, userApiKeys?: Record<string, string>) {
  ...
  switch (provider) {
    ...
    case 'OpenCode':
      return env.OPENCODE_API_KEY || cloudflareEnv.OPENCODE_API_KEY;
    case 'OpenAILike':
...
export function getBaseURL(cloudflareEnv: Env, provider: string) {
  switch (provider) {
    ...
    case 'OpenCode':
      return env.OPENCODE_API_BASE_URL || cloudflareEnv.OPENCODE_API_BASE_URL || 'https://api.opencode.ai/v1';
    case 'OpenAILike':
